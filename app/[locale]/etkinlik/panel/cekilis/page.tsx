...
import { CheckCircle2, Gift, LockKeyhole, ShieldCheck } from "lucide-react";
...
export default async function RafflePage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ durum?: string }> }) {
  ...
  if (quizGate.required && !quizGate.completed) return <main className="mx-auto flex min-h-[70vh] max-w-2xl items-center px-5 py-16"><div className="w-full rounded-[2rem] border border-primary-100 bg-card p-8 text-center shadow-xl sm:p-10 dark:border-white/10"><LockKeyhole className="mx-auto size-14 text-accent" /><h1 className="mt-5 font-heading text-3xl font-bold">{en ? "Complete the quiz to unlock the raffle" : "Çekilişi açmak için önce quiz'i tamamla"}</h1><p className="mt-4 leading-7 text-muted-foreground">{en ? `${quizGate.answeredQuestions} of ${quizGate.totalQuestions} questions completed. Finish them all to earn your raffle entry.` : `${quizGate.totalQuestions} sorunun ${quizGate.answeredQuestions} tanesini tamamladın. Çekiliş hakkını kazanmak için hepsini yanıtla.`}</p><Button asChild size="lg" className="mt-7"><Link href={en ? "/en/event/hub/quiz" : "/etkinlik/panel/quiz"}>{en ? "Continue the quiz 🧠" : "Quiz'e devam et 🧠"}</Link></Button></div></main>;
