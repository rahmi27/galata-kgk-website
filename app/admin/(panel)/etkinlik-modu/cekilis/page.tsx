...
import { drawRaffleWinnerAction, saveRaffleNameAction } from "@/app/admin/(panel)/etkinlik-modu/cekilis/actions";
...
export default async function RaffleAdminPage({ searchParams }: { searchParams: Promise<{ durum?: string }> }) {
  ...
  return <>
    <AdminPageHeader eyebrow="Etkinlik Modu" title={active?.raffleName ?? "Çekiliş Yönetimi"} description="Katılım koşullarını kabul eden adaylar arasından kriptografik güvenli rastgele seçim yapın." actions={<Button asChild variant="outline"><Link href="/admin/etkinlik-modu"><ArrowLeft /> Merkeze dön</Link></Button>} />
    ...
    {durum === "ad-kaydedildi" ? <p className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-900">Çekiliş adı kaydedildi.</p> : null}
    {durum === "gecersiz-ad" ? <p className="mt-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-900">Çekiliş adı en az 3 karakter olmalıdır.</p> : null}
    {durum === "oturum-yok" ? <p className="mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900">Adı kaydedilecek aktif oturum bulunamadı.</p> : null}
    {!active ? <p className="mt-8 rounded-3xl border border-dashed p-10 text-center">Aktif etkinlik oturumu bulunmuyor.</p> : <div className="mt-8 grid gap-7 lg:grid-cols-2">
      <section className="rounded-3xl border border-primary-100 bg-white p-6 dark:border-white/10 dark:bg-primary-950 lg:col-span-2">
        <h2 className="font-heading text-xl font-bold">Çekiliş ayarları</h2>
        <form action={saveRaffleNameAction} className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="block flex-1 text-sm font-semibold">Çekiliş adı<Input name="raffleName" defaultValue={active.raffleName} minLength={3} maxLength={120} required className="mt-2" /></label>
          <Button type="submit">Adı Kaydet</Button>
        </form>
        <p className="mt-2 text-xs text-primary-500 dark:text-primary-200">Aynı anda yalnızca aktif etkinlik oturumunun tek çekilişi yönetilir.</p>
      </section>
      <section className="rounded-3xl border border-primary-100 bg-white p-6 dark:border-white/10 dark:bg-primary-950">
        <div className="flex items-center justify-between gap-4"><div><h2 className="font-heading text-xl font-bold">{active.raffleName} havuzu</h2><p className="mt-1 text-sm text-primary-500">{active.raffleEntries.length} katılımcı</p></div><form action={drawRaffleWinnerAction}><Button type="submit" disabled={!active.raffleEntries.length}><Sparkles /> Kazananı Çek</Button></form></div>
