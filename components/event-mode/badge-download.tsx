...
export function BadgeDownload(props: BadgeDownloadProps) {
  ...
  return <div>
    <header className="mb-7 text-center"><p className="text-sm font-bold uppercase tracking-[.2em] text-accent-700 dark:text-accent-300">{en ? "A memory worth sharing" : "Paylaşmaya değer bir anı"}</p><h1 className="mt-3 font-heading text-4xl font-bold">{en ? "Claim Your Badge! 🎉" : "Rozetini Kap! 🎉"}</h1><p className="mt-3 leading-7 text-muted-foreground">{en ? `Show everyone you were part of ${eventTitle}—share it in your story!` : `${eventTitle}'na katıldığını göster, story'nde paylaş!`}</p></header>
