...
import { deleteSponsorTierAction } from "@/app/admin/(panel)/sponsorlar/kategoriler/actions";
...
export function DeleteSponsorTierButton({
  tierId,
  tierName,
  sponsorCount,
}: {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deleteSponsorTierAction(tierId);
      ...
      <DialogContent className="rounded-2xl border-primary-100 bg-white dark:border-white/15 dark:bg-primary-950">
        <DialogHeader>
          <DialogTitle className="font-heading text-xl text-primary-950 dark:text-white">
            Sponsor tier’ı silinsin mi?
          </DialogTitle>
          <DialogDescription className="leading-6 text-primary-500 dark:text-primary-200">
            “{tierName}” tier’ı silinecek.
            {sponsorCount > 0
              ? ` Bu tier'a bağlı ${sponsorCount} sponsor olduğu için işlem engellenecek.`
