...
export function MembershipEditForm({ action, role, roleEn = "", order, membershipId }: { action: (state: AdminActionState, formData: FormData) => Promise<AdminActionState>; role: string; roleEn?: string; order: number; membershipId: number }) {
  const [state, formAction, isPending] = useActionState(action, initialAdminActionState);
  return (
    <form action={formAction} className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_7rem_auto] sm:items-end">
      ...
      <FormField label="Rol — İngilizce (opsiyonel)" htmlFor={`membership-role-en-${membershipId}`}><Input id={`membership-role-en-${membershipId}`} name="roleEn" defaultValue={roleEn} minLength={2} maxLength={140} /></FormField>
      ...
      <div className="sm:col-span-4"><ActionMessage state={state} /></div>
    </form>
  );
}
