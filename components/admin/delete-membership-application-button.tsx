...
import { deleteMembershipApplicationAction } from "@/app/admin/(panel)/katilim-basvurulari/actions";
...
export function DeleteMembershipApplicationButton({
  applicationId,
  applicantName,
}: {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deleteMembershipApplicationAction(applicationId);
      ...
      <DialogContent className="rounded-2xl border-primary-100 bg-white dark:border-white/10 dark:bg-primary-950">
        <DialogHeader>
          <DialogTitle className="font-heading text-xl text-primary-950 dark:text-white">
            Başvuru kalıcı olarak silinsin mi?
          </DialogTitle>
          <DialogDescription className="leading-6 text-primary-500 dark:text-primary-200">
            “{applicantName}” adına kayıtlı katılım başvurusu ve içerdiği kişisel
            bilgiler silinecek. Bu işlem geri alınamaz.
          </DialogDescription>
        </DialogHeader>
        {error ? (
