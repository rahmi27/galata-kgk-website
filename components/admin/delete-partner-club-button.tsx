...
import { deletePartnerClubAction } from "@/app/admin/(panel)/is-birlikleri/actions";
...
export function DeletePartnerClubButton({
  partnerClubId,
  partnerClubName,
  collaborationCount,
}: {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deletePartnerClubAction(partnerClubId);
      ...
      <DialogContent className="rounded-2xl border-primary-100 bg-white dark:border-white/15 dark:bg-primary-950">
        <DialogHeader>
          ...
          <DialogDescription className="leading-6 text-primary-500 dark:text-primary-200">
            ...
            maddesi kalıcı olarak silinecek.
          </DialogDescription>
        </DialogHeader>
        {error ? (
